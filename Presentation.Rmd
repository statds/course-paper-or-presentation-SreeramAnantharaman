...
- Engle and Russell (1998) \cite{ACD} introduced the ACD model, which presents a new way of modelling random durations arising from high frequency data. 

- ACD models was developed to study the dynamic structure of durations in the context of irregularly spaced financial transactions data. 
...
- Also we have shown an example where we fit an Log-ACD model for modeling wave height durations. 

- This is a very different example of the application of duration models as its mainly used for modeling financial application. 
...
- The Autoregressive Conditional Duration model or ACD model were proposed by Engle and Russell (1998) \cite{ACD}. They are defined in terms of conditional density of durations. 
...
where $\epsilon_i ~$ i.i.d non negative random variable with $E(\epsilon_i)=1$ and density function $f_{\epsilon}$. And 

$\psi_i = \omega + \sum_{j=1}^m \alpha_j x_{i-j} + \sum_{j=1}^q \beta_j \psi_{i-j}$

With the following constraints on the coefficients: $\omega > 0$, $\beta \geq 0$, $\alpha \geq 0$ and $\sum_{j=1}^m\alpha + \sum_{j=1}^q\beta < 1$ to ensure non-negativity and weak stationarity of the durations $x_i$.
...
are independent and identically distributed, that is iid with $E(\epsilon_i)=1$

Let $f_{\epsilon}$ be the density of $\epsilon$. 
...
If we assume that the error follow an exponential distribution then the likelihood function for the ACD model is:
...
- In order to accommodate for that alternate distribution free approaches were introduced. 
...
- This data can be obtained from two main portals: the National Oceanic and Atmospheric Administration (NOAA) and the United States Army Corps of Engineers (USACE) databases. 
...
The dataset contains the following variables: Year, Month, Day, Hour, Wave height, Wind speed, Fetch, Wind speed square, Z2, Z3 and Z4. 
...
- So the data is available for each hour from the 11-01-2004 to 10-31-2013. 

- Another important thing to note is each year can be divided into two periods: (i) Windy Months (Nov-Mar) (ii) Calm Months (Apr-Oct). 
...
- That is the durations are calculated as the difference in the hourly time for a wave to reach a threshold. 
...
```{r  label = "4ft durations", echo = FALSE, fig.cap = "Wave duration at 4ft threshold", out.width = '95%'}
...
And the missing data in the exogenous variables were backfilled. 
